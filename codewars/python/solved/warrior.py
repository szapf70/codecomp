...
class Warrior:
    ...
    def calcRank(self, level):
        return level // 10
    
    def updateRank(self):
        ranks = ["Pushover", "Novice", "Fighter","Warrior","Veteran","Sage", 
                 "Elite","Conqueror","Champion","Master","Greatest"]
        self.rank = ranks[self.calcRank(self.level)]

    def updateExp(self,exp):
        self.experience += exp
        if self.experience < 100: self.experience = 100
        if self.experience > 10_000: self.experience = 10_000
        self.updateLevel()
    ...
    def battle(self,enemyLvl):
        preself = self.level
        print(f"{enemyLvl} - {self.level}/{self.rank}")
        if not 1 <= enemyLvl <= 100:
            return "Invalid level"
        if self.calcRank(self.level) < self.calcRank(enemyLvl) and self.level < enemyLvl-4:
            return "You've been defeated"
        if self.level - 1 == enemyLvl:
            self.updateExp(5)
        if self.level == enemyLvl:
            self.updateExp(10)

        ldiff = enemyLvl - self.level
        
        if self.level < enemyLvl:
            self.updateExp(20*ldiff**2)

        if preself < enemyLvl:
            return "An intense fight"        
        
        if preself == enemyLvl or preself -1 == enemyLvl:
            return "A good fight"
        
        if preself >= enemyLvl +2:
            return "Easy fight"

    def training(self, trData):
        aText,exp, mLvl = trData
        if self.level < mLvl:
            return "Not strong enough"
        self.updateExp(exp)
        self.achievements.append(aText)
        return aText
...
The greater the difference between levels, the more experinece your warrior will gain. The formula is 20 * diff * diff where diff 
equals the difference in levels between the enemy and your warrior.

However, if your warrior is at least one rank lower than your enemy, and at least 5 levels lower, your warrior cannot fight against 
an enemy that strong and must instead return "You've been defeated".

Every successful battle will also return one of three responses: "Easy fight", "A good fight", "An intense fight". Return "Easy fight" 
if your warrior is 2 or more levels higher than your enemy's level. Return "A good fight" if your warrior is either 1 level higher or equal to 
your enemy's level. Return "An intense fight" if your warrior's level is lower than the enemy's level.
Logic Examples:
...
If a warrior level 8 fights an enemy level 13, they will receive 0 experience points and return "You've been defeated". (Remember, difference in rank & enemy 
...
Training will accept an array of three elements (except in java where you'll get 3 separated arguments): the description, the experience 
points your warrior earns, and the minimum level requirement.
If the warrior's level meets the minimum level requirement, the warrior will receive the experience points from it and store the description 
of the training. It should end up returning that description as well.
If the warrior's level does not meet the minimum level requirement, the warrior doesn not receive the experience points and description and 
